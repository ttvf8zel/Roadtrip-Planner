...
export default function StopsPanel({ stops, routeCache, onEdit, onDelete, onMove, onAdd, onReset, onFocus }) {
            ...
            <div key={s.id}>
              ...
              <div style={{ background:'#07111a',border:'1px solid #1e3a4a',borderRadius:9,padding:'10px 12px',marginBottom:5,transition:'border-color .15s' }}
                ...
                <div style={{ display:'flex',alignItems:'center',gap:7,marginBottom:4 }}>
                  ...
                  <div style={{ fontSize:13,fontWeight:600,color:'#e8dcc8',flex:1,minWidth:0,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',cursor:'pointer' }} onClick={()=>onFocus(s)}>{s.name.split(',')[0]}</div>
                </div>
                ...
                <div style={{ display:'flex',gap:3,marginTop:7,paddingLeft:29 }}>
                  {[['✏️',()=>onEdit(s)],['🗺',()=>onFocus(s)],['↑',()=>onMove(s.id,-1)],['↓',()=>onMove(s.id,1)],['🗑',()=>onDelete(s.id)]].map(([lbl,fn],idx)=>(
